...
namespace opp_k8 {
__device__ inline void get_sigma_ef_sq_kernel(
    const double* val,
    double* max_val
) {
    for (int dim = 0; dim < 3; ++dim) {
        *max_val += (val[(dim) * opp_k8_dat0_stride_d] * val[(dim) * opp_k8_dat0_stride_d]);
    }
}
}

//--------------------------------------------------------------
__global__ void opp_dev_get_sigma_ef_sq_kernel(
...
) 
{
    ...
    for (int d = 0; d < 1; ++d)
        opp_reduction<OPP_INC>(gbl1 + blockIdx.x * 1 + d, gbl1_local[d]);
...
) 
{
    ...
    if (iter_size > 0) 
    {
        ...
        {
            opp_dev_get_sigma_ef_sq_kernel<<<num_blocks, block_size, (reduction_size * block_size)>>>(
