...
) 
{
    ...
    if (iter_size > 0) {
        ...
        opp_queue->submit([&](sycl::handler &cgh) {
            ...
            // -----------------------------------------------------------------------------------------
            auto  update_pos_kernel_sycl = [=](const double* part_vel, double* p_pos)
            {
                for (int dm = 0; dm < 2; dm++) {

                    p_pos[(dm) * opp_k1_dat1_stride_sycl[0]] += part_vel[(dm) * opp_k1_dat0_stride_sycl[0]] * CONST_dt_sycl[0]; // s1 = s0 + ut
