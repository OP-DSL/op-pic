...
for run in 1 2; do
    for config in 6144000 12288000 24576000 49152000; do
        ...
        # ---------------------
        command=${binPath}${binName}' '${currentfilename}' >'$folder'/log_N'${num_nodes}'_C'${config}'_D10_R'${run}'.log 2>&1'
        echo "RUNNING ->"$command
        srun $command | tee $folder/log_N${num_nodes}_C${config}_D10_R${run}.log;

        sed -i "s/REAL plasma_den     = 1e18/REAL plasma_den     = 0.5e18/" ${currentfilename}
        command=${binPath}${binName}' '${currentfilename}' | tee '$folder'/log_N'${num_nodes}'_C'${config}'_D5_R'${run}'.log;'
        echo "RUNNING ->"$command
        srun $command | tee $folder/log_N${num_nodes}_C${config}_D5_R${run}.log;

        sed -i "s/REAL plasma_den     = 0.5e18/REAL plasma_den     = 1e17/" ${currentfilename}
        command=${binPath}${binName}' '${currentfilename}' | tee '$folder'/log_N'${num_nodes}'_C'${config}'_D1_R'${run}'.log;'
        echo "RUNNING ->"$command
        srun $command | tee $folder/log_N${num_nodes}_C${config}_D1_R${run}.log;
        # ---------------------
