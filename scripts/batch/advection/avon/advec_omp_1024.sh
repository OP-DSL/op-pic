...
runFolder=$PWD"/OMP_ADV_NODE1_L_"$(date +"D_%Y_%m_%d_T_%I_%M_%S")
...
for run in 1 2; do
    for config in 1024; do
        for tasks in 48 24 12 8 4 1; do
            ...
            srun $command | tee $folder/log_N${tasks}_C${config}_E3_R${run}.log;

            sed -i "s/REAL dt = 0.001/REAL dt = 0.01/" ${currentfilename}
            command=${binPath}${binName}' '${currentfilename}
            echo "RUNNING dt = 0.01 ->"$command
            srun $command | tee $folder/log_N${tasks}_C${config}_E2_R${run}.log;

            sed -i "s/REAL dt = 0.01/REAL dt = 0.1/" ${currentfilename}
            command=${binPath}${binName}' '${currentfilename}
            echo "RUNNING dt = 0.1 ->"$command
            srun $command | tee $folder/log_N${tasks}_C${config}_E1_R${run}.log;
            # ---------------------
        done
    done
done
