...
# runFolder=$PWD"/TESOS_MPI_ADV_NODE1_S_"$(date +"D_%Y_%m_%d_T_%I_%M_%S")
...
#             mpirun -np $tasks $command | tee $folder/log_N${tasks}_C${config}_E3_R${run}.log;
...
#             mpirun -np $tasks $command | tee $folder/log_N${tasks}_C${config}_E2_R${run}.log;
...
#             mpirun -np $tasks $command | tee $folder/log_N${tasks}_C${config}_E1_R${run}.log;
#             # ---------------------
#         done
#     done
# done
...
runFolder=$PWD"/TELOS_OMP_2_ADV_NODE1_S_"$(date +"D_%Y_%m_%d_T_%I_%M_%S")
echo "Creating running folder -> " $runFolder
