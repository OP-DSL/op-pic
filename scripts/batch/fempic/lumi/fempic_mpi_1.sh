...
#SBATCH --time=0-04:00:00            # Run time (d-hh:mm:ss)
...
for run in 1 2 3; do
    for gpus in 1; do
        for config in 24000 48000 96000; do
            ...
            srun --cpu-bind=${CPU_BIND} -n $totalGPUs ${binary} ${currentfilename} | tee $folder/log_N${num_nodes}_G${totalGPUs}_C${config}_SR${use_seg_red}_D10_R${run}.log;
