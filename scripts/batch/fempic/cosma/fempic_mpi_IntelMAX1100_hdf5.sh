...
runFolder=$PWD"/SYCL_SR"${use_seg_red}"_"$(date +"D_%Y_%m_%d_T_%I_%M_%S")
    ...
    for config in 96000 48000; do
        for gpu_red_arrays in 1024 512 256 128 64 32 16 8 4 2 1; do
            for run in 1 2 3; do
                ...
                mpirun -np $gpus ./igpu_wrapper.sh $binary $currentfilename > $folder/log_G${gpus}_M${actual_config}_D${plasma_den}_ARR${gpu_red_arrays}_R${run}.log;
            done
        done
    done
done
