...
runFolder=$PWD"/CUDA_1NODE_"$(date +"D_%Y_%m_%d_T_%I_%M_%S")
...
for run in 1 2; do
    for gpus in 1 2 4; do
	    ...
	    (( totalGPUs=$gpus*$SLURM_JOB_NUM_NODES ))
            ...
            bede-mpirun --bede-par 1ppg -np $totalGPUs ${binary} ${currentfilename} | tee $folder/log_N${num_nodes}_G${totalGPUs}_C${config}_D10_R${run}.log;

            sed -i "s/REAL plasma_den     = 1e18/REAL plasma_den     = 1.3e18/" ${currentfilename}
       	    echo "RUNNING d 13e17 ->"$totalGPUs $command
            bede-mpirun --bede-par 1ppg -np $totalGPUs ${binary} ${currentfilename} | tee $folder/log_N${num_nodes}_G${totalGPUs}_C${config}_D13_R${run}.log;
	    # ---------------------
