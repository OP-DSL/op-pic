...
free -h >> memory_usage_$SLURM_NODEID.log
...
monitor_memory() {
    while true; do
        free -h >> memory_usage_$SLURM_NODEID.log  # Using SLURM_NODEID to differentiate logs
        sleep 1
    done
}
...
for run in 1; do # 2
    for config in 192000 ; do #384000
