...
) 
{
    ...
    if (opp_k3_map0_stride != args[0].size) {
        ...
        cutilSafeCall(cudaMemcpyToSymbol(opp_k3_map0_stride_d, &opp_k3_map0_stride, sizeof(OPP_INT)));
    }
    ...
    if (iter_size > 0) 
    {
        ...
        else // Do segmented reductions ----------       
        {
            if (opp_k3_sr_set_stride != set->size) {
                ...
                cutilSafeCall(cudaMemcpyToSymbol(opp_k3_sr_set_stride_d, &opp_k3_sr_set_stride, sizeof(OPP_INT)));
            }
