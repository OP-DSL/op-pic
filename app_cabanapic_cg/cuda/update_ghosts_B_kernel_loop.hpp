...
) 
{
    ...
    opp_mem::dev_copy_to_symbol<OPP_INT>(opp_k5_map0_stride_d, &opp_k5_map0_stride, &(args[2].size), 1);
    ...
    if (iter_size > 0) 
    {
        ...
        {
            opp_dev_update_ghosts_B_kernel<<<num_blocks, block_size, 0, *opp_stream>>>(
