...
) 
{
    ...
    if (opp_k6_map0_stride != args[2].size) {
        ...
        cutilSafeCall(cudaMemcpyToSymbol(opp_k6_map0_stride_d, &opp_k6_map0_stride, sizeof(OPP_INT)));
    }
    ...
    if (iter_size > 0) 
    {
        ...
        else // Do segmented reductions ----------       
        {
            if (opp_k6_sr_set_stride != set->size) {
                ...
                cutilSafeCall(cudaMemcpyToSymbol(opp_k6_sr_set_stride_d, &opp_k6_sr_set_stride, sizeof(OPP_INT)));
            }
